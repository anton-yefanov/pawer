/**
 * Shared pieces of the mascot uploader API (tools/image-uploader/).
 * See assets/masters/README.md for the master spec these enforce.
 */
...
/** Vercel buffers the request body itself for image/*; this is the fallback. */
 ...
 * rather than a rep.
 */
export async function boundingBoxDrift(png, siblingUrl) {
  if (!siblingUrl) return null;
  try {
    const sibling = Buffer.from(await (await fetch(siblingUrl)).arrayBuffer());
