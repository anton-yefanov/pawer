...
- A template or folder card's cover is a mesh gradient, not a theme token: `templates.color` / `folders.color` hold an id from `src/constants/card-colors.ts` (null means grey), `cardGradient()` in `src/lib/card-gradients.ts` is the only thing that turns one into colours, and `CardCover` in `src/components/templates/card-cover.tsx` is the only thing that draws one. `expo-mesh-gradient` renders nothing below iOS 18, so the flat fill `CardCover` paints under the mesh is the cover on older devices, not a backdrop. The covers are pastel in both schemes — the artwork sits directly on them — so there is no dark variant. App-shipped templates ship a hue from the seed and keep it; users set their own through the Customize sheet (`src/app/(workout)/customize.tsx`). Every cover is the *front face of the folder illustration*: the nine mesh colours are sampled off `assets/images/folders/<color>.webp` at the nine vertices, so a cover cannot drift from the icon beside it — regenerate them by sampling the icons, never by hand-picking a ramp. The points are symmetric with the middle row low, which is what reads as a lit panel rather than a wash. A photo cover is the one case where the hue is real but invisible, and why the Customize sheet's swatch row dims instead of disappearing (`hidesCardColor()`).
