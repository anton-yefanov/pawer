...
`tools/image-uploader/` is the page for filling the masters: each of the 205 exercises next to its two upstream reference photos, with drop/paste slots that validate against the master spec before writing. The in-app preview shows the exercise's two frames — the uploaded masters, or the upstream photos while a frame is still empty. `npm run images:web` runs it — `server.mjs`, a plain `node:http` server on port 4000 that serves the static page and writes drops straight into `assets/masters/`. Git is the only store; `npm run build:images` is the whole path from a drop to the app.

Nothing about it touches the app bundle, but one of its constraints shapes the code: reference photos are **not** in this repo, they come straight from jsDelivr over the public `yuhonas/free-exercise-db`, so a missing photo is an `img` error, never a server-side existence check. `master-spec.mjs` holds the validation — the executable half of what `assets/masters/README.md` specifies — and the server is what makes the slots' state just `fs.stat` on the master file.
