...
Illustrations are never referenced by path from a screen. `src/lib/exercise-images.ts`, `src/lib/mascot-images.ts` and `src/lib/attribute-images.ts` are the only resolvers (Metro needs static `require` literals, so any real lookup map must be generated or written out). `exercise-images.ts` reads `src/lib/exercise-image-map.ts`, which `scripts/build-images.mjs` regenerates from the masters that actually built — a slug missing a frame maps to `null` and the exercise sheet draws a grey square in its place. Mascot art is still placeholders, and is now only the analytics cards — card covers carry emoji instead.
...
**Vercel Blob is the store, not git.** A frame is three things in the `pawer-blob` store: the source exactly as it was dropped at `masters/sources/<sourceId>_<frame>.png`, the 1200×1200 square rendered from it at `masters/exercises/<sourceId>_<frame>.png`, and its crop rectangle in `masters/crops.json`. **Only the square ever ships**, and it is always `renderSquare()` of a crop over a source — so a drop may be any aspect ratio, and reframing re-cuts the square from the source rather than re-encoding a square. `npm run masters:pull` mirrors the squares into `assets/masters/exercises/` for `npm run build:images`, and the page's Export button is the same thing as a zip; `npm run masters:push` is the other direction, for seeding the store from masters git still holds. Anything outside Vercel needs `BLOB_READ_WRITE_TOKEN` in `.env.local`; the deployment gets it from the project.

Reframing happens in the preview tile itself, not a modal: the square keeps its place in the phone layout while the whole source hangs out of it dimmed, so what sits inside the outline is exactly what ships. The crop is held in **source pixels** and `reframeScale()` is the only thing that converts it to the tile, which is what keeps drag, the four corner handles and the saved rectangle in one coordinate space.
...
- **A cover's artwork is up to three emoji, stored as JSON in `templates.artwork`.** `src/lib/card-artwork.ts` is the only thing that parses or writes that column, and it is a discriminated union rather than a bare id so pre-defined stickers can join emoji without another migration; `null` is the bare gradient, which is what Clear produces. `ArtworkLayer` in `src/components/templates/artwork-layer.tsx` draws it and owns the one/two/three sizing, which is why every caller hands it a `coverHeight` instead of letting it measure — a measured row draws a frame late and a grid of those flickers on scroll.
